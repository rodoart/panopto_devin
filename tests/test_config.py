...
from panopto.config.tables import ProcessConfig
...
def test_settings_from_env_defaults():
    ...
    assert settings.hdfs_staging_base == "/tmp/panopto_test_staging"


def test_settings_from_env_overrides(monkeypatch):
    """Environment variables override the Settings defaults."""
    monkeypatch.setenv("PANOPTO_ENV", "prod")
    monkeypatch.setenv("PANOPTO_POSTGRES_HOST", "pg.example.com")
    monkeypatch.setenv("PANOPTO_POSTGRES_PORT", "5433")
    monkeypatch.setenv("PANOPTO_HDFS_STAGING_BASE", "/prod/staging")
...
def test_process_config_from_json_values_and_env_override(tmp_path, monkeypatch):
    """JSON values are loaded and PANOPTO_HDFS_* env variables override them."""
    ...
    monkeypatch.setenv("PANOPTO_HDFS_STAGING_BASE", "/env/staging")
    monkeypatch.setenv("PANOPTO_HIVE_WAREHOUSE_DIR", "/env/warehouse")
