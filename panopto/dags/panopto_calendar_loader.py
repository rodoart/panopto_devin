"""DAG de Airflow panopto_calendar_loader; expone las funciones external_calendar_ready, convert_external_to_hive, sync_hive_to_postgres, pause_dependent_dags, send_red_alert, calendar_load_failure."""
...
def external_calendar_ready(**context: Any) -> bool:
    ...
    spark = SparkSessionBuilder(app_name="panopto_calendar_loader_check").build()
    try:
        count = spark.sql(f"""
...
def convert_external_to_hive(**context: Any) -> None:
    ...
    spark = SparkSessionBuilder(app_name="panopto_calendar_loader_convert").build()
    spark.sql(f"""
...
def sync_hive_to_postgres(**context: Any) -> None:
    ...
    spark = SparkSessionBuilder(app_name="panopto_calendar_loader_sync").build()
...
def pause_dependent_dags(context: Any) -> None:
    ...
    try:
        ...
        for dag_model in session.query(DagModel).all():
            if dag_model.dag_id != current and dag_model.dag_id.startswith("panopto_"):
