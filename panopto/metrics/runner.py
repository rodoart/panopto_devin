...
from panopto.metrics.summary import VariableSummaryBuilder
    ...
    def run(
        ...
        for row in variables:
            ...
            if table_config is None:
                raise MissingDataError(f"table metadata not found for {row['source_table']}")
            reading_mode = str(table_config.reading_mode or "each")
            info_col = table_config.date_column
