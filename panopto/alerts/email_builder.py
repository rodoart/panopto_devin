...
class EmailBuilder:
    ...
    def _default_config() -> Dict[str, Any]:
        ...
        path = os.environ.get("PANOPTO_EMAIL_CONFIG_PATH", "config/email_config.json")
    ...
    def build_html(
        ...
        title = f"{self.config.get('subject_prefix', '[PANOPTO]')} Alerta {model_id} - {information_date}"
    ...
    def _build_body(
        ...
        parts = [
            f"<h1>Alerta PANOPTO: {escape(model_name)} ({escape(model_id)})</h1>",
